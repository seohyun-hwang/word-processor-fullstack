...
public class BackendApplication {
		...
		static final short lineWidthPxMax = 512; // 2^9
		...
		public static void insertChar(
				byte userIndex,
				Character insertedChar, Integer positionOfInsertion,
				Byte charWidthPx, Byte charHeightPx)
		{
		...
		public static void reStyleSelectedChar_bold(
				int positionOfSelectedChar, Byte appliedBolding)
		{
		...
		public static void reStyleSelectedChar_italic(
				int positionOfSelectedChar, Byte appliedItalic)
		{
		...
		public static void reStyleSelectedChar_underline(
				int positionOfSelectedChar, Byte appliedUnderline)
		{
		...
		public static void reStyleSelectedChar_typeface(
				int positionOfSelectedChar, String appliedTypeface,
				Byte charWidthPxDefault)
		{
		...
		public static void reStyleSelectedChar_heightInPx(
				int positionOfSelectedChar, Byte appliedCharHeightPx)
		{
				print("updating the typeface at the position-of-selected.", true);
				mainCharArray.get(positionOfSelectedChar).setCharHeightPx(appliedCharHeightPx);
				print("typeface-update successful.", true);
		}
		public static void reColorSelectedChar_text(
				int positionOfSelectedChar, String appliedTextColor)
		{
		...
		public static void reColorSelectedChar_highlight(
				int positionOfSelectedChar, String appliedHighlightColor)
		{
		...
		public static void clearCopiedCharStack(
				byte userIndex)
		{
		...
		public static void addToCopiedCharStack(
				byte userIndex,
				int positionOfCopiedChar)
		{
		...
		public static void undo_aModification(
				byte userIndex)
		{
				print("undoing a modification.", true);
				if (
						!mainUserArray.get(userIndex).getModificationTypeStack().isEmpty()
						&& (mainUserArray.get(userIndex).getModificationTypeStack().size() - mainUserArray.get(userIndex).getUndoCount()) > 0
						&& mainCharArray.size() > 2)
				{
		...
		public static void redo_anUndoneModification(
				byte userIndex)
		{
	...
	public static byte getUndoModificationType(
			byte userIndex)
	{
		print("signal of modification-type of undo.", true);
		if (
				!mainUserArray.get(userIndex).getModificationTypeStack().isEmpty()
						&& (mainUserArray.get(userIndex).getModificationTypeStack().size() - mainUserArray.get(userIndex).getUndoCount()) > 0
						&& mainCharArray.size() > 2)
		{
			if (mainUserArray.get(userIndex).getUndoCount() < mainUserArray.get(userIndex).getUndoCount_ceiling()) {
				print("undo-count: " + mainUserArray.get(userIndex).getUndoCount(), true);
				print("modification-type stack size: " + mainUserArray.get(userIndex).getModificationTypeStack().size(), true);
				...
				String typeOfUndoneModification = mainUserArray.get(userIndex).getModificationType_ofStack(positionOf_latestModification - undoCount);
				if (typeOfUndoneModification.equalsIgnoreCase("insert")) {
					print("undo type signaling successful.", true);
					return 1;
				} else if (typeOfUndoneModification.equalsIgnoreCase("delete")) {
					print("undo type signaling successful.", true);
					return -1;
	...
	public static int getUndoModificationPosition(
			byte userIndex)
	{
		print("signal of modification-position of undo.", true);
		if (
				!mainUserArray.get(userIndex).getModificationTypeStack().isEmpty()
						&& (mainUserArray.get(userIndex).getModificationTypeStack().size() - mainUserArray.get(userIndex).getUndoCount()) > 0
						&& mainCharArray.size() > 2)
		{
			if (mainUserArray.get(userIndex).getUndoCount() < mainUserArray.get(userIndex).getUndoCount_ceiling()) {
				...
				String typeOfUndoneModification = mainUserArray.get(userIndex).getModificationType_ofStack(positionOf_latestModification - undoCount);
				Integer positionOfUndoneModification = mainUserArray.get(userIndex).getModificationPosition_ofStack(positionOf_latestModification - undoCount);
				print("undo position signaling successful.", true);
				return positionOfUndoneModification;
			} else {
				print("reached the maximum undo-count.", true);
			}
		} else {
	...
	public static byte getRedoModificationType(
			byte userIndex)
	{
		print("signal of modification type of the undo being redone.", true);
		short undoCount = mainUserArray.get(userIndex).getUndoCount();
		if (mainUserArray.get(userIndex).getUndoCount() > 0) {
			mainUserArray.get(userIndex).setUndoCount((short) (undoCount - 1));
			print("undo-count (after redo): " + undoCount, true);
			...
			String typeOfUndoneModification = mainUserArray.get(userIndex).getModificationType_ofStack(positionOf_latestModification - undoCount);
			if (typeOfUndoneModification.equalsIgnoreCase("insert")) {
				print("redone undo type signaling successful.", true);
				return 1;
			} else if (typeOfUndoneModification.equalsIgnoreCase("delete")) {
				print("redone undo type signaling successful.", true);
				return -1;
	...
	public static int getRedoModificationPosition(
			byte userIndex)
	{
		print("signal of modification position of the undo being redone.", true);
		short undoCount = mainUserArray.get(userIndex).getUndoCount();
		if (mainUserArray.get(userIndex).getUndoCount() > 0) {
			mainUserArray.get(userIndex).setUndoCount((short) (undoCount - 1));
			print("undo-count (after redo): " + undoCount, true);
			...
			int positionOf_latestModification = mainUserArray.get(userIndex).getModificationTypeStack().size() - 1;
			Integer positionOfUndoneModification = mainUserArray.get(userIndex).getModificationPosition_ofStack(positionOf_latestModification - undoCount);
			print("redone undo position signaling successful.", true);
			return positionOfUndoneModification;
		} else {
			print("nothing to redo.", true);
		}
		...
		public static void createNewElement_mainCharArray(int count) {
				...
				for (int i = 0; i < count; i++) {
						mainCharArray.add(new MainCharArray(null, null, null, null, null,null, "TimesNewRoman", null, null, null, null, null));
				}
		}
		...
		public static void modificationTrackingTruncation(byte userIndex) {
				short undoCount = mainUserArray.get(userIndex).getUndoCount();
		...
		public static void modificationTrackingContribution(
				byte userIndex,
				Integer positionOfTargetedElement, String typeOf_latestModification)
		{
